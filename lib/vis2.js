    ...
    legend.append("text")
       ...
       /* pieChartSvg
          ...
          .join('polyline')
            ...
            .outerRadius(radius*.99).centroid(d) // line break: we use the other arc generator that has been built only for that
              const posC = d3.arc()
            .innerRadius(radius*.99)
            .outerRadius(radius*.19).centroid(d); // Label position = almost the same as posB
       ...
       /* pieChartSvg
          ...
          .join('text')
            ...
            .style('text-anchor', function(d) {
                const midangle = d.startAngle + (d.endAngle*0.19 - d.startAngle) / 2
